...
public class Store implements IBlobStore {
    private static final String TAG = "s3.Store",
                                PROTOCOL = "http",
     ...
     */
    // XXX should be created using an Account!
    public Store(String key, String keyId) {
        mKey = key;
        mKeyId = keyId;
    }
    ...
    private void logProblem(int status, HttpURLConnection connection) throws IOException {
          Log.e(TAG, "" + status + connection.getResponseMessage());
          //Log.d(TAG, new String(slurpStream(connection.getErrorStream())));
    }
}
